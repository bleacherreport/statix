...
defmodule StatixTest do
  ...
  test "service_check/2,3" do
    ...
    service_check("sample name", :ok)
    assert_receive {:test_server, _, "_sc|sample name|0"}

    service_check("sample name", :ok, timestamp: now_unix)
    s = "_sc|sample name|0|d:#{now_unix}"
    assert_receive {:test_server, _, ^s}

    service_check("sample name", :ok, hostname: "sample.hostname")
    assert_receive {:test_server, _, "_sc|sample name|0|h:sample.hostname"}

    service_check("sample name", :ok, tags: ["foo", "bar"])
    assert_receive {:test_server, _, "_sc|sample name|0|#foo,bar"}

    service_check("sample name", :ok, message: "sample message")
    assert_receive {:test_server, _, "_sc|sample name|0|m:sample message"}

    service_check("sample name", :warning,
      timestamp: now_unix,
      hostname: "H",
      tags: ["F", "B"],
      message: "M"
  ...
  test "port closed" do
    ...
    assert capture_log(fn ->
             assert {:error, :port_closed} == event("sample", "test")
           end) =~ "event metric \"sample\" lost value test due to port closure"

    assert capture_log(fn ->
             assert {:error, :port_closed} == service_check("sample", "check")
           end) =~ "service_check metric \"sample\" lost value check due to port closure"
  end
end
